...
2. [x] Batch 2: persistent conversational coach. **Implemented, awaiting reviewer acceptance.** Delivered:
   ...
   - `ChatRepository` (sessions CRUD, bounded recent-messages window) and `CoachChatService`: each user message snapshots the deterministic coach context (same builder as Batch 1), sends [rules + context] in the stable system prefix + last `MAX_CHAT_HISTORY_TURNS` (20) messages + the new message (user prompt) to the provider, and persists both turns; unknown session → 404 `SESSION_NOT_FOUND`.
   - Prompt-cost optimization: `serializeCoachContext` slims the serialized context (null/empty values and dataless daily-status entries omitted; meaningful zeros kept — a strict subset of the structured preview, no new keys); the context JSON lives in the first (system) message, so the prefix is byte-identical across turns whenever training data is unchanged and provider-side prompt caching serves it instead of re-billing the full context every turn. Tests: prefix byte-stability across turns, prefix recomputation after training data changes, per-turn user prompt contains only the raw message, history never carries context JSON.
...
3. [x] Batch 3: AI training-plan drafts. **Implemented, awaiting reviewer acceptance.** Delivered:
   - `aiPlanDraftRequestSchema` (`horizonDays: 7|14` default 7, optional ≤500-char instruction) and `aiPlanDraftResponseSchema` (items ≤ `MAX_AI_PLAN_DRAFT_ITEMS` = 14, each `AiPlanDraftItem` mapping 1:1 onto `plannedWorkoutCreateSchema`; horizon, window dates, model, generatedAt). `POST /api/ai/coach/plan-draft` is user-triggered only; it sends the same deterministic coach context (slim-serialized) plus a structured-output system contract to the provider, Zod-parses the reply at the boundary (code fences tolerated), and rejects out-of-window dates, invalid workout types, >14 items, empty items, and malformed JSON as stable sanitized errors (502 `AI_INVALID_OUTPUT`/`AI_EMPTY_RESPONSE`, 400 request, 503 disabled, 502/504/429 provider).
   - `/coach/plan` page (link 计划草稿 from the coach page): preview with AI 生成 label, model name and generation time and the draft window; every field editable (date/type/title/distance/duration), per-item include checkbox; 导入所选 calls the existing `POST /api/planned-workouts` per confirmed item (allSettled, partial failures reported and kept for retry) and invalidates calendar/calendar-plan/training-summary. The draft endpoint never writes to SQLite; a draft is never canonical until imported. Entering the page or switching the horizon never calls the provider.
   - Tests: parser unit tests (valid/fenced/malformed/empty/oversized/out-of-window), integration (prompt carries no names/notes/device, read-only row counts, disabled/invalid/provider-failure mapping, draft item imports through the existing API and appears in the calendar), and an E2E scenario (explicit generate with armed 429 failure → retry, preview/edit, import selected item, calendar verification, own-data cleanup) against the local mock provider.
...
- [x] M7 Batch 3: AI training-plan drafts — structured Zod-validated output, /coach/plan preview/edit, import via the existing planned-workout API (implemented, awaiting reviewer acceptance; M7 as a whole is not yet accepted).
