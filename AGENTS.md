...
M5 Batch 4 (implemented, awaiting reviewer acceptance) is the daily-loop regression closeout: Calendar mutations now also invalidate the `['calendar-plan']` prefix so the Dashboard's daily cards see plan create/edit/complete/relink/delete immediately; CSV/FIT imports and pending-import resolutions also invalidate `['dashboard']` and `['calendar-plan']`; daily-status and settings mutations already invalidated prefixes the Dashboard shares. A full real-flow closed-loop E2E (dashboard canonical today → create today's plan → record today's status → CSV import dated today → manual link & complete in the calendar → dashboard shows 已完成 + linked activity + weekly distance; reload persists; a 409 relink failure never fakes success; the test cleans only its own data) guards the loop. Daily status never feeds medical conclusions, readiness scores, or automatic plan adjustments. Do not add AI-generated training plans, DeepSeek/OpenAI or other model integrations, ParroTao online sync, watch or Garmin Connect writes, daily check-ins (in the social sense), authentication/multi-user, cloud deployment, social features, online maps, installers, or medical conclusions.
