...
M6 Batch 1 (reviewer-accepted, verdict PASS WITH FOLLOW-UP) delivers the bounded training context and server-side read-only DeepSeek integration: `POST /api/ai/context` returns the context preview (plus a SHA-256 `contextFingerprint`) **without ever calling the provider**, and `POST /api/ai/review` (body `{windowDays: 7|28, contextFingerprint}`) assembles `AiTrainingContext` exclusively from existing deterministic aggregates — Monday-start weekly volumes **fully contained** in the window (partial boundary weeks dropped, never clipped), dashboard 7/28-day run summaries, and training-summary counts. The whitelist structurally excludes raw imports, samples, GPS coordinates, daily-status scales, free-text notes, and secrets; running aggregates may still be regarded as health-related data, and the server enable switch only permits calls — it is not the user's per-send confirmation. The injectable `TrainingReviewProvider` interface has a DeepSeek chat-completions adapter (model `deepseek-flash` per the official docs) — real calls require `RUNCOACH_AI_ENABLED=true` + `RUNCOACH_AI_PROVIDER=deepseek` + a non-empty `RUNCOACH_DEEPSEEK_API_KEY` (blank key = absent, never blocks startup), disabled by default; the key never reaches the frontend, logs, or Git. Stable sanitized errors: 503 `AI_DISABLED`, 400 `INVALID_AI_REVIEW_REQUEST`, 409 `AI_CONTEXT_STALE`, 504 `AI_TIMEOUT`, 429 `AI_RATE_LIMITED`, 502 `AI_PROVIDER_ERROR`/`AI_EMPTY_RESPONSE`/`AI_INVALID_OUTPUT` (review text ≤ `MAX_AI_REVIEW_CHARS` = 4000). Zod-validated; no code path writes to SQLite; no chat history table. AI output is informational only — never medical advice, readiness scores, or automatic plan adjustments.

M6 Batch 5 (implemented, awaiting reviewer acceptance) adds the settings-UI DeepSeek key configuration: `/settings` has an “AI 回顾配置（DeepSeek）” section saving the key via `PUT /api/settings/ai-key` into `ai-provider.json` inside the data directory (atomic, gitignored, restart-persistent); `GET /api/settings/ai` returns `{aiEnabled, provider, source: 'file'|'env'|null, maskedTail}` and the full key is never echoed to the client, logged, or committed; resolution priority is settings-UI key file > env key > disabled, and a UI key enables real calls without the `RUNCOACH_AI_ENABLED` flag while sending still requires the per-request preview + fingerprint confirmation on `/review`. The env configuration path stays unchanged. Do not move the key into browser storage, add more providers, or auto-send.

M7 (approved by the owner, 2026-09-26) is the AI 训练助手: a conversational coach that may read the full training context — including daily-status scales and notes, which the owner explicitly authorized as the highest-sensitivity part — hold persistent SQLite chat memory, and generate training-plan drafts that the user previews and confirms before they are imported through the existing planned-workout API. Batch 1 (implemented, awaiting reviewer acceptance) delivers `AiCoachContext` + `GET /api/ai/coach-context`: all-time RUN totals, deterministic personal bests, bounded recent-60 activity summaries, 52 weekly volumes, 28-day plan summary, and the authorized 28-day daily-status window; it is read-only, never calls the provider, and is previewable on `/review`. Still never sent: GPS tracks, raw samples, raw imports, device info, activity names. Metrics stay deterministic; the LLM only explains. Batch 2 (implemented, awaiting reviewer acceptance) adds the persistent conversational coach: migration `0006_ai_chat.sql` (`chat_sessions`/`chat_messages` with cascade delete), `ChatRepository` with a bounded recent-messages window, `POST /api/ai/coach/chat` + session list/messages/delete APIs, and the `/coach` page (nav 教练) — each user message snapshots the Batch-1 context and persists both turns; typing or browsing never sends, and the provider interface carries the bounded history. Batch 3 (plan drafts + import) is not started.

M6 Batch 2 (reviewer-accepted, verdict PASS WITH FOLLOW-UP) turns this into the 训练回顾 page (`/review`, nav 回顾): (`/review`, nav 回顾): the window selector (7/28 days) only fetches the read-only preview; entering the page, switching the window, or refreshing never calls `/api/ai/review`; the preview shows the exact date range, every field that will be sent (including `eligibleCount` and an expandable raw-context JSON identical to the prompt payload), and a clear “数据将发送到 DeepSeek 云端” notice with the terms caveat. The confirm button is disabled while the preview loads/refetches/failed, while a request is pending (double-click guard), or while AI is disabled (with a hint on how to enable); each explicit confirmation sends one request carrying the preview's fingerprint, and the server rejects stale confirmations with 409 `AI_CONTEXT_STALE` before contacting the provider — the page then clears the old confirmation, re-fetches the preview in-page, and requires a fresh explicit confirmation (retryable errors like 429 keep their per-click retry button); no long-lived authorization state exists. The result area labels the text as AI 生成 with the actual model name and generation time; program-computed stats stay in the preview panel (canonical) and the AI text is explicitly not canonical data and never medical advice. E2E: a `review` project runs against a local mock DeepSeek provider (`e2e/mock-deepseek.mjs`, no real key — this is NOT a real DeepSeek API test) exercising the real adapter/config/guards (failure→retry, double-click guard, stale-fingerprint rejection with in-page recovery, cancel = zero calls) and a `review-disabled` project covers the preview-only/disabled state; `scripts/run-e2e.mjs` wipes `.e2e-data-review` and `.e2e-data-review-disabled` before and after runs so repeats stay clean; CI never depends on a real key. Do not add chat/memory, AI-generated plans, background automatic calls, browser-side keys, or extra providers. Do not add AI-generated training plans, additional LLM providers, chat/memory features, automatic data sending, ParroTao online sync, watch or Garmin Connect writes, daily check-ins (in the social sense), authentication/multi-user, cloud deployment, social features, online maps, installers, or medical conclusions.
