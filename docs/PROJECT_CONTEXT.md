...
> 当前基线：`master` 分支，当前里程碑 M6（有界训练上下文与 DeepSeek 服务端只读接入）；M1–M5 均已完成并冻结，M6 Batch 1 与 Batch 2 均已验收（结论均为 PASS WITH FOLLOW-UP）；M6 阶段验收由 reviewer 决定。此前版本基于更早里程碑编写。
...
- M6 Batch 1 已验收（结论 PASS WITH FOLLOW-UP）：
  ...
  - 真实 API 冒烟证据（**用户手动执行**，非实现 agent 运行）：在隔离的合成数据目录中配置本地 key 后，回顾页返回 `deepseek-flash` 的结果，浏览器中 `POST /api/ai/review` 返回 200，截图显示请求约 2.59 秒。未提交或分享 key、原始请求体或个人数据。CI 中的 E2E 使用本地 mock provider，与此真实调用相互独立。M6 整体阶段验收由 reviewer 决定。
...
- Batch 2（已实现，等待 reviewer 验收）：持久对话教练。`0006_ai_chat.sql`（chat_sessions + chat_messages，role CHECK、级联删除、索引）；`ChatRepository`（会话 CRUD、有界最近消息窗口）；`CoachChatService`（每条用户消息：解析/创建会话 → 快照确定性教练上下文 → 发送 [规则+上下文]（system 前缀）+ 最近 20 条消息 + 新消息（user prompt）→ 持久化双方轮次；未知会话 404 `SESSION_NOT_FOUND`）。Prompt 成本优化：`serializeCoachContext` 精简序列化（省略 null/空值、空数组与无数据的每日状态条目，保留有意义的 0；为结构化预览的严格子集、不新增任何键），上下文 JSON 位于首条 system 消息——训练数据未变化时逐轮字节一致，命中 provider 侧前缀缓存，避免每轮重复计费完整上下文；user prompt 仅含用户原始消息，history 不含上下文。测试覆盖：跨轮前缀字节一致、数据变化后前缀重算、history 无上下文泄漏。API：`POST /api/ai/coach/chat`、`GET /api/ai/coach/chat/sessions`、`GET/DELETE .../sessions/:sessionId(/messages)`，Zod 校验（消息 ≤2000 字符，回复 ≤4000）。`/coach` 页（导航“教练”）：会话列表/删除、消息流（AI 生成标注）、输入与 pending/error/retry 状态；刷新后会话与消息仍在；输入与浏览不会触发发送。provider 接口扩展有界 `history`。仅用户显式发送才调用模型。
- Batch 3（已实现，等待 reviewer 验收）：AI 训练计划草稿。`POST /api/ai/coach/plan-draft`（仅用户显式触发；`horizonDays: 7|14` 默认 7，可选 ≤500 字偏好说明）把同一确定性教练上下文（精简序列化）+ 结构化输出契约发给 provider，回复在服务边界用 Zod 解析（容忍代码块围栏）；items ≤ 14 条，每条与 `plannedWorkoutCreateSchema` 一一对应；日期必须落在 [today, today+horizon-1]，越界/非法类型/超量/空列表/畸形 JSON 分别映射为稳定脱敏错误（502 `AI_INVALID_OUTPUT`/`AI_EMPTY_RESPONSE`、400 请求无效、503 未启用、502/504/429 provider）。`/coach/plan` 页（教练页“计划草稿”入口）：预览带 AI 生成标注、模型名、生成时间与草稿区间；所有字段可编辑、逐条勾选导入；导入所选经**既有** `POST /api/planned-workouts` 逐条写入（allSettled，部分失败保留重试），成功后失效 calendar/calendar-plan/training-summary 查询。草稿端点绝不写 SQLite，草稿在导入前不是 canonical 数据；进入页面或切换范围不调用 provider。测试：解析器单测、集成（prompt 无名称/备注/设备、行数不变、错误映射、草稿项经既有 API 导入并出现在日历）、E2E（armed 429 失败→重试、预览编辑、导入、日历验证、自建数据清理，本地 mock provider）。
