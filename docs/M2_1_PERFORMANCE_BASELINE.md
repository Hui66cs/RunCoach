...
- 测试提交 SHA：`d68aeea`（M2.1）。生产代码自 `e5e7239`（M2完成）以来未改动；本批返工仅调整基准工具本身（单一测量容器、显式预热、结构自检）。
- Node：v24.18.0
- 平台：win32 x64，Windows 10.0.26200
- CPU：AMD Ryzen 7 8845HS
- 运行方式：仓库根目录 `pnpm benchmark:m2`
- 基准总耗时：约 7.9 s；临时数据库自动清理，`cleanupVerified: true`
- 结构自检：基准对 JSON 输出做结构校验（operations 数量/规模完整、每项 median/min/max 有限非负且 min≤median≤max、cleanupVerified、必要字段齐全），共 51 项校验，任一失败退出码非 0。
...
- 每项操作先预热 1 次（不计入统计；预热结果同时用于正确性校验），再正式测量（读操作 7 次、写操作 3 次），报告 median/min/max；计时使用 `performance.now()`。写操作以第 1 次创建为预热，其后 3 次创建计入统计。
...
| ------------------------------------------- | -------- | ----------: | -------: | -------: | -----------: |
| createActivity                              | 10,000   |      223.45 |   204.75 |   229.03 |           33 |
| createActivity                              | 50,000   |     1037.34 |  1034.56 |  1044.00 |          113 |
| listActivitiesPage 首页                     | 8 个活动 |        0.32 |     0.29 |     0.46 |            3 |
| listActivitiesPage（date/type/source 筛选） | 8 个活动 |        0.37 |     0.35 |     0.42 |            3 |
| getActivity                                 | 10,000   |       22.07 |    20.77 |    23.90 |            8 |
| getActivity                                 | 50,000   |      153.50 |   148.37 |   170.50 |            8 |
| getActivitySeries 全范围                    | 10,000   |       16.71 |    16.32 |    20.71 |            2 |
| getActivitySeries 全范围                    | 50,000   |       93.96 |    86.93 |   114.41 |            2 |
| getActivitySeries 范围 10%（4500..5500）    | 10,000   |        2.08 |     1.85 |     2.52 |            2 |
| getActivitySeries 范围 10%（22500..27500）  | 50,000   |        8.46 |     8.26 |    10.02 |            2 |
...
1. `getActivity` 随样本数近线性增长：10k 约 22 ms，50k 约 154 ms（约 7 倍），而查询次数固定为约 8 条——增长来自单条大结果集的处理而非查询次数。
2. `/series` 全范围同样近线性：10k 约 17 ms，50k 约 94 ms（约 5.6 倍），查询次数仅约 2 条。
3. 范围过滤到 10% 后耗时降至全范围的约 1/8–1/11（2.1 ms / 8.5 ms），说明 SQL 范围过滤本身有效，series 的成本与范围内行数成正比。
