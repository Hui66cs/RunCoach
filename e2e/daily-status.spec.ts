...
test('settings page edits and persists the athlete profile without touching other settings', async ({
  ...
  // Clear the optional profile fields and verify the cleared state persists.
...
test('daily status page creates, edits, isolates per date, and deletes entries', async ({
  ...
  // Switch to another day: the empty form must not leak the previous date.
  ...
  await expect(page.getByRole('radio', { name: '睡眠质量 4' })).not.toBeChecked();
  await expect(page.getByRole('button', { name: '删除当天记录' })).toHaveCount(0);

  // All-empty save is blocked with guidance instead of creating a null row.
  await page.getByRole('button', { name: '保存', exact: true }).click();
  await expect(page.getByTestId('daily-status-error')).toHaveText(
    '请先填写至少一项内容；如需清空当天记录，请使用删除按钮。',
  );

  // Returning to the original date shows its own persisted record.
